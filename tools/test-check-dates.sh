...
set +e
"$TOOL" > /tmp/test_check-dates_$$.txt 2>&1
rc=$?
set -e
...
if [ "$rc" -eq 0 ] || [ "$rc" -eq 1 ]; then
    ...
    tail -20 /tmp/test_check-dates_$$.txt | sed 's/^/    /'
    rm -f /tmp/test_check-dates_$$.txt
    exit 1
fi
rm -f /tmp/test_check-dates_$$.txt
