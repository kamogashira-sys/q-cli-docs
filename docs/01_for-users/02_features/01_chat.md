...
| コマンド | 詳細説明 | 使用シーン |
|---------|---------|-----------|
| `/knowledge show` | 現在のKnowledge情報を表示。インデックス化されたファイル数、合計サイズ、最終更新日時を確認できる | プロジェクトのどのファイルがAIの参照対象になっているか確認したい時 |
...
| `/knowledge status` | バックグラウンドで実行中のインデックス化処理の進行状況を表示 | 大量のファイルを追加した後、処理が完了したか確認したい時 |
