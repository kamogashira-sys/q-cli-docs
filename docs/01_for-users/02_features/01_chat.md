...
| `/prompts [サブコマンド]` | 再利用可能なプロンプトテンプレートを管理。MCPサーバーから提供されるプロンプトやローカルプロンプトを利用可能。`@<prompt name>`でプロンプトを直接取得できる | 頻繁に使う指示をテンプレート化したい時、MCPサーバーのプロンプトを利用したい時 |
...
| `/hooks` | 設定されているコンテキストフックを表示 | 現在のフック設定を確認したい時、フックの動作を理解したい時 |
| `/editor [初期テキスト]` | $EDITORでプロンプトを作成（デフォルト: vi） | 長いプロンプトを書く時、使い慣れたエディタで編集したい時 |
