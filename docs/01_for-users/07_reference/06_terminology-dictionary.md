...
| バージョン | 日付 | 変更内容 |
|-----------|------|---------|
| v2.1.0 | 2025-10-18 | サイト全体の用語チェックに基づき6用語追加（Server, Default, Version, Global, User, Token）<br/>表形式に変更（可読性向上、75%の行数削減）<br/>注記列を横並びに変更（GitHub互換性向上）<br/>注記を記号化（*1: 英語のまま使用、*2: Rust型名は英語のまま使用）<br/>使用していないカテゴリでは凡例を非表示 |
