...
> **Note**: 本サイトではv1.13.0以降のコマンドを対象に記述しています。
...
**全26コマンド**: 基本操作(6) / 会話管理(4) / Agent・設定(4) / タスク管理(2) / Knowledge・情報管理(2) / 開発者向け(5) / 情報表示(4) / 問題報告(1)
...
**引数**:
- `[コマンド名]`: 詳細ヘルプを表示するコマンド名（省略時は全コマンド一覧）

**使用例**:
- `/help`: 全コマンド一覧を表示
- `/help agent`: `/agent`コマンドの詳細ヘルプを表示
...
**目的**: クリップボードから画像を貼り付ける

**使用シーン**: 画像をチャットに追加する / スクリーンショットを共有する

**使用方法**: `/paste`

**注意点**: 
- クリップボードに画像がコピーされている必要がある
- スクリーンショットやキャプチャした画像を直接貼り付け可能
...
**目的**: 再利用可能なプロンプトテンプレートを管理する

**使用シーン**: プロンプトを管理・利用したい時

**使用方法**: `/prompts [サブコマンド]`

**サブコマンド**:
- `list`: 利用可能なプロンプト一覧を表示
- `details <name>`: 特定のプロンプトの詳細情報を表示
- `get <name> [arg]`: プロンプトを取得（`@<name>`でも可）
- `create`: 新しいローカルプロンプトを作成
- `edit`: 既存のローカルプロンプトを編集
- `remove`: 既存のローカルプロンプトを削除

**ショートカット**:
- `@<prompt name> [arg]`: プロンプトを直接取得（`/prompts get`と同等）

**注意点**: 
- MCPサーバーから提供されるプロンプトとローカルプロンプトの両方を管理
- プロンプトは共通のワークフローやタスクへの素早いアクセスを提供

**関連コマンド**: `/agent`, `/model`, `/mcp`
...
#### `/subscribe` - Proサブスクリプション

**目的**: Q Developer Proサブスクリプションへのアップグレード

**使用シーン**: Proプランにアップグレードする / クエリ制限を増やす

**使用方法**: `/subscribe [オプション]`

**オプション**:
- `--manage`: AWSコンソールを開いて既存のサブスクリプションを管理

**注意点**: 
- クエリ制限を増やすことができる
- Proプランの詳細は[料金プラン比較](../05_deployment/02_pricing-comparison.md)を参照
...
**使用シーン**: ツールの確認・権限設定をしたい時

**使用方法**: `/tools [サブコマンド]`

**サブコマンド**:
- `schema`: 全ツールの入力スキーマを表示
- `trust <tool>`: 特定のツールを信頼（セッション中は確認不要）
- `untrust <tool>`: ツールの信頼を解除（確認が必要に戻る）
- `trust-all`: 全ツールを信頼（非推奨の`/acceptall`と同等）
- `reset`: 全ツールをデフォルトの権限レベルにリセット

**注意点**: 
- デフォルトでは、特定のツール使用時に確認を求める
- `trust`で信頼したツールは、セッション中は確認不要
...
**目的**: $EDITOR環境変数で指定されたエディタでプロンプトを作成

**使用シーン**: 長いプロンプトを書く時 / 使い慣れたエディタで編集したい時

**使用方法**: `/editor [初期テキスト]`

**引数**:
- `[INITIAL_TEXT]`: エディタに初期表示するテキスト

**注意点**: 
- $EDITOR環境変数で指定されたエディタを使用（デフォルト: vi）
- エディタの設定: `export EDITOR=vim`などで環境変数を設定

**使用例**:
```bash
/editor              # エディタでプロンプトを作成
/editor "初期テキスト"  # 初期テキストを指定
```
...
**目的**: 最新のアシスタントメッセージを引用形式で$EDITORに読み込み、返信を作成する

**使用シーン**: 長文の返信を書く時 / 使い慣れたエディタで編集したい時

**使用方法**: `/reply`

**注意点**: 
- $EDITOR環境変数で指定されたエディタを使用
- 最新のアシスタントメッセージが引用形式で読み込まれる
