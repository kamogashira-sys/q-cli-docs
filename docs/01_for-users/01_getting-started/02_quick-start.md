...
コマンドが正しくインストールされていれば、パスが表示されます。

> 💡 詳細なパス情報は [インストールガイド - インストール後のファイル配置](01_installation.md#-インストール後のファイル配置)を参照

### トラブルシューティング

**コマンドが見つからない場合**:
1. シェルを再起動: `exec $SHELL`
2. パスを確認: `echo $PATH`
3. [インストールガイド](01_installation.md)を参照
