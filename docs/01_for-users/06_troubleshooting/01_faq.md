[ホーム](../../README.md) > [ユーザーガイド](../README.md) > [トラブルシューティング](README.md) > 01 Faq
...
**中級者向け（3-5時間）**:
1. [Agent設定](../03_configuration/03_agent-configuration.md)
2. [MCP設定](../03_configuration/04_mcp-configuration.md)
3. [設定例集](../03_configuration/08_examples.md)
...
詳細は[優先順位ルール](../03_configuration/07_priority-rules.md)を参照。
...
詳細は[MCP設定](../03_configuration/04_mcp-configuration.md)を参照。
...
詳細は[Agent設定](../03_configuration/03_agent-configuration.md)を参照。
...
詳細は[MCP設定](../03_configuration/04_mcp-configuration.md#トラブルシューティング)を参照。
...
### コンテキストウィンドウのサイズを変更できますか？

**A**: いいえ、コンテキストウィンドウのサイズはモデルによって決定され、ユーザーが変更することはできません。

**詳細**:
- デフォルト: 200,000トークン
- モデルによって異なる（Claude Sonnet 4: 200,000トークン、GPT: 128,000トークンなど）
- より大きなコンテキストウィンドウが必要な場合は、対応するモデルを選択してください
...
**参考**: [コンテキスト管理ガイド](../08_guides/README.md)
...
**対処法**:
- 不要なファイルを削除: `/context remove <pattern>`
- すべてクリア: `/context clear`
- 強制追加（注意）: `/context add --force <file>`

**参考**: [コンテキスト管理ガイド](../08_guides/README.md)

### メッセージサイズの制限を変更できますか？

**A**: いいえ、メッセージサイズの制限はハードコードされており、変更できません。
...
**参考**: [設定リファレンス](../07_reference/03_settings-reference.md)

### 自動要約を無効化すべきですか？

**A**: 推奨しません。自動要約を無効化すると、コンテキストウィンドウがオーバーフローした際にエラーが発生する可能性があります。
...
詳細は[Agent設定](../03_configuration/03_agent-configuration.md#️-ツール設定)を参照。
