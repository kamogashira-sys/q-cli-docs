...
**レベル2: コマンドライン引数**
- **設定場所**: `q chat`コマンドの引数
- **制御項目**: Agent選択（`--agent <NAME>`）
- **変更方法**: コマンド実行時に指定
- **優先度**: データベース設定・ファイル設定より優先

**レベル3: データベース設定**
- **設定場所**: `~/.local/share/amazon-q/settings.json`
- **制御項目**: デフォルトAgent（`chat.defaultAgent`）
- **変更方法**: `q settings chat.defaultAgent <NAME>`
- **優先度**: ファイル設定より優先

**レベル4: ファイルベース設定**
- **設定場所**: Agent設定ファイル、レガシーMCP設定ファイル
- **制御項目**: MCP/Agent詳細設定
- **変更方法**: JSONファイルを直接編集
- **優先度**: 次項の「MCP設定ファイルの優先順位」を参照

**レベル5: デフォルト値**
- **設定場所**: Q CLIのソースコード
- **制御項目**: 全設定のデフォルト値
- **変更方法**: 変更不可
- **優先度**: 最低（他の設定がない場合のみ使用）
