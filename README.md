...
Kiro CLI v2.x 系では **Windows対応・Headless Mode・新TUI** を中心に大幅な機能拡張が行われ、**v2.8.0 では CLI v3（Early Access）** が登場しました：

1. **[v2.8.0 / v2.8.1 CLI v3 Early Access](kiro-docs/02_update/01_changelog.md)**（2026-06-17）
...
2. **[v2.7.0 /goal・Queue Steering・enriched /rewind](kiro-docs/02_update/01_changelog.md)**（2026-06-12）
...
3. **[v2.6.1 Linux: libasound.so.2 依存除去](kiro-docs/02_update/01_changelog.md)**（2026-06-08）
   - Linux ビルドが起動時に `libasound.so.2` を要求しなくなる（オーディオ依存パッケージ不要）

4. **[v2.6.0 Transcript Export・/title・永続化](kiro-docs/02_update/01_changelog.md)**（2026-06-05）
...
5. **[v2.5.1 APIエンドポイント移行](kiro-docs/02_update/01_changelog.md)**（2026-06-01）
   - API エンドポイントを `*.kiro.dev` へ移行（firewall 許可リスト要確認）

6. **[v2.5.0 Thinking Display・Review Loops](kiro-docs/02_update/01_changelog.md)**（2026-05-29）
...
7. **[v2.4.2 Windowsクラッシュ修正](kiro-docs/02_update/01_changelog.md)**（2026-05-26）
   - Windows 環境で発生していた koffi/createRequire 起因クラッシュ修正（terminal input handling）

8. **[v2.4.1 MCP環境変数展開修正](kiro-docs/02_update/01_changelog.md)**（2026-05-21）
   - MCPサーバー設定での `${VAR_NAME}` 環境変数展開構文の修正

9. **[v2.4.0 /rewind・/effort・/settings](kiro-docs/02_update/01_changelog.md)**（2026-05-20）
...
10. **[v2.3.0 MCP OAuth・KIRO_HOME・Keybindings](kiro-docs/02_update/01_changelog.md)**（2026-05-12）
...
11. **[v2.2.2 MCPガバナンス強化](kiro-docs/02_update/01_changelog.md)**（2026-05-05）
   - V2 TUIモードでのMCP governance強制適用（エンタープライズ・API keyユーザー）

12. **[v2.2.1 UX改善・安定性修正](kiro-docs/02_update/01_changelog.md)**（2026-05-04）
   - `chat.disableWrap`設定追加、`/model set-current-as-default`の保存先変更、9件のバグ修正

13. **[v2.2.0 Adaptive Thinking](kiro-docs/02_update/01_changelog.md)**（2026-04-27）
   - マルチターン会話でモデルの推論を保持し応答品質を向上

14. **[v2.1.0 Shell Streaming・Tool Search](kiro-docs/02_update/01_changelog.md)**（2026-04-24）
   - シェルコマンド出力のリアルタイムストリーミング
   - Skills as Slash Commands（`/skill-name`で直接呼び出し）
   - Device Flow認証（ポートフォワーディング不要）

15. **[v2.0.0 Windows・Headless Mode](kiro-docs/02_update/01_changelog.md)**（2026-04-13）
   - Windows 11ネイティブ対応
...
**主要ドキュメント**:
- **[機能詳細ガイド](kiro-docs/01_features/README.md)** - 30機能の詳細解説
