...
> **注記**: 本ページは **v2.19.1**（2026-08-21）まで反映しています。**v2.8.0**（2026-06-17）で **Kiro CLI v3（Early Access）** が `--v3` により先行公開されました（→ [09_v3/](../09_v3/README.md)）。最新版は[公式 Changelog](https://kiro.dev/changelog/cli/) を参照してください。
>
> **本節の掲載範囲**: 目安として直近 3〜4 バージョンを掲載し、それより古いものは新しいバージョンの追加時に[バージョン履歴](#バージョン履歴)へ移動します。

### v2.19.1 CLI（2026-08-21）

**主要な変更**: tmux でのオーバーフローフラッシュリプレイ回避や `/knowledge` サブコマンド自動補完復元などのバグ修正と、`.kiroignore` で拒否されたパスの漏出を防ぐセキュリティ修正が中心。

**バグ修正（7件）**:
- 📡 tmux < 3.7 など synchronized output 非対応ターミナルで、scrollback preservation 有効時の overflow-flush replay を回避。
- 🔧 `/knowledge` サブコマンドの自動補完と `rm` エイリアスを V3 agent で復元。
- 🎨 TUI 終了後に端末のテキスト属性（下線等）が親シェルへ残留しないよう修正。
- 🔧 [V3] 決定を永続化できないシェルコマンドで「Always allow」が提示されないよう修正。
- 📡 [V3] 接続切断（`EPIPE`）でリトライするよう修正（生の `write EPIPE` エラーを表面化させない）。
- 🔧 [V3] 出力トークン上限超過でツール呼び出しが打ち切られた際のリトライループを修正。
- ⚡ [V3] initialize 中の experiment 解決待ちによるセッション起動レイテンシの regression を修正。

**セキュリティ（1件）**:
- 🔒 **[V3] `grep_search`/`file_search` の `.kiroignore` 漏出防止**: `.kiroignore` で拒否されたパスを返さないよう修正。

**出典**: `kiro-cli version --changelog=all`（`kiro-cli version --changelog=2.19.1`）

**注記**: ※公式Changelogページ（[v2.19](https://kiro.dev/changelog/cli/2-19/)）の "Latest Patches" タブに日付（Aug 21, 2026）のみ掲載され、本文には専用セクションがありません。詳細内容は CLI 内蔵changelogのみで確認しています。

---

### v2.19.0 CLI（2026-08-19）

**主要な変更**: サブエージェントのアイドルタイムアウト設定（`api.subagentTimeout`）と spec review 画面へのマウス対応の追加、およびストリーム安定性・認証周りの多数のバグ修正が中心。

**機能追加（4件）**:
- 🔧 **サブエージェントのアイドルタイムアウト（`api.subagentTimeout`）**: 停止したサブエージェントが親ターンをハングさせなくなり、子エージェントごとにアイドル期限（既定 3600秒）が設定可能に。詳細: [02. Subagents](../01_features/02_Subagents.md)
- 🎨 **Spec review screen のマウス対応**: スクロールでのナビゲーションとクリックでのカーソル位置指定に対応、`m` キーでトグル。詳細: [18. Terminal UI](../01_features/18_TerminalUI.md)
- 📋 **[V3] セッション再開ピッカーのAI生成タイトル**: session resume ピッカーに最初のプロンプトを要約した短いAI生成タイトルが表示され、似たセッションを識別しやすくなった。詳細: [09_v3/](../09_v3/README.md)
- 🔌 **[V3] MCP protocol revision 2026-07-28 対応**: このprotocol revisionを要求するMCPサーバーが接続可能に。詳細: [09_v3/](../09_v3/README.md)

**変更（2件）**:
- ⚙️ エージェント実行中に入力したスラッシュコマンドが自動実行されるように変更: 読み取り専用コマンドは即時、それ以外はターン終了時に実行。
- ⚙️ 利用不可なツールへの呼び出しが、ターン終了まで持ち越されず即時に失敗するよう変更。

**バグ修正（25件）**:
- 📡 応答ストリームが停止したままターンがハングしないよう、アイドル監視（60秒で警告、300秒でキャンセル）を追加。`api.streamIdleSoftTimeout`/`api.streamIdleHardTimeout` で調整可能。
- 📡 スロットリング・5xx・接続切断（ストリーム中の切断含む）を自動的にバックオフ付きでリトライし、リトライバナーに表示。
- 📡 ストリーミング応答の既定タイムアウトを、より短いリクエストタイムアウトではなく 3600秒（60分）の壁時計タイムアウトに変更（長い応答が途中で切られなくなった）。`api.timeout` で上書き可能（より短い値への変更含む）。
- 🔧 コンテキストウィンドウのオーバーフローでリトライループに陥らず、自動圧縮が発火するよう修正。
- 🔧 圧縮自体がオーバーフローした場合でも、最も古い履歴を破棄してコンテキストオーバーフローから復帰できるよう修正。
- 🔐 ターン中の認証期限切れでターンが終了せず、トークンを更新してリトライするよう修正（V2/V3）。
- 🔐 更新でバイナリの実行位置が変わった後も認証リフレッシュとセッションコマンドが動作するよう修正。
- ⚙️ `/goal` が一時的なエラーをバックオフ付きでリトライするよう修正（停止しない）。
- 💡 goal 実行中の `Ctrl+C` がCLI全体を終了せず、goal をキャンセルするよう修正。
- 🔧 認識されないスラッシュコマンドが、モデルへのチャットとして送信されず拒否されるよう修正。
- 💡 サブエージェントの承認待ち中にアクティブなツールを表示するよう修正。
- 🎤 音声入力ヘルパーの失敗が「No speech detected」と誤表示されないよう修正。
- 🔧 `/context clear` で削除したエントリが、agent設定の再読み込み時に再表示されないよう修正。
- 🔧 agent設定で以前削除したリソースを再宣言すると再アタッチされるよう修正。
- 🔧 キャンセルされたhookが、同じhookの後続実行に影響しないよう修正。
- 🔧 `fileMatch`・`manual` inclusionのsteeringファイルが、globマッチで自動注入されないよう修正。
- 🔐 レジストリMCPサーバーが既定のOAuthスコープを注入しないよう修正（Atlassian authv2認証の修正）。
- 🔧 終了時に余分な文字がシェルへリークしないよう修正。
- 🔧 [V3] 確認・エラーメッセージが次の入力まで表示され続けるよう修正。
- 🔧 [V3] `/model` で利用不可なモデルを選んだ際、アクティブなモデルが保持されるよう修正。
- 🔒 [V3] ファイルシステム権限ルールがディレクトリルートに正しくマッチするよう修正。
- 🔧 [V3] JSON agentプロファイルの任意文字列フィールドで `null` が受け付けられるよう修正。
- 🔌 [V3] セルフホストMCPレジストリのカタログ取得が403を返さないよう修正。
- 🔧 [V3] steering文書とスラッシュコマンドがセッション間でリークしないよう修正。
- 💡 [V3] 起動直後、モデルロード未完了時にモデル一覧が空にならないよう修正。

**出典**: `kiro-cli version --changelog=all`（`kiro-cli version --changelog=2.19.0`）、[公式Atomフィード](https://kiro.dev/changelog/feed.atom)、[公式Changelog v2.19](https://kiro.dev/changelog/cli/2-19/)

**注記**: CLI内蔵changelogの日付は2026-08-18ですが、公式サイト表示日（2026-08-19）を採用しています（1日差異）。
...
**最終更新**: 2026-08-22（v2.19.1・v2.19.0対応追加、v2.17.0をバージョン履歴へ移動、冒頭注記を v2.19.1 に更新。v2.19.0の`api.subagentTimeout`は→[02_Subagents.md](../01_features/02_Subagents.md)、spec review screenのマウス対応は→[18_TerminalUI.md](../01_features/18_TerminalUI.md)、[V3] session resumeタイトル要約・MCP protocol revision対応は→[09_v3/README.md](../09_v3/README.md)に詳細解説を追加。v2.19.1は公式Changelogページの本文に専用セクションがなく"Latest Patches"タブに日付のみのため、CLI内蔵changelogのみを一次情報として採用。前回 2026-08-16: v2.18.1・v2.18.0・v2.17.0対応追加、v2.16.2・v2.16.1・v2.16.0をバージョン履歴へ移動、冒頭注記を v2.18.1 に更新。v2.18.0の`/voice`（オンデバイス音声入力）は→[37_VoiceMode.md](../01_features/37_VoiceMode.md)、v2.17.0の Cloud Sessions（プレビュー）は→[36_CloudSessions.md](../01_features/36_CloudSessions.md)に詳細解説ページを追加。v2.18.0の Cloud Sessions 既定オプトイン化は破壊的変更として明記（公式Web表現を採用、内蔵changelogの governance controls 表現とは異なる）。v2.18.1は公式Changelogサイト未掲載のためCLI内蔵changelogのみを一次情報として採用。前回 2026-08-01: v2.16.0・v2.15.2・v2.15.1・v2.15.0対応追加、v2.14.2・v2.14.1・v2.14.0・v2.13.1をバージョン履歴へ移動、冒頭注記を v2.16.0 に更新。v2.16.0の`/tangent`（V3）は既存classic版`/tangent`（`04_reference/02_slash-commands.md`）と同名別仕様のため、両者へ相互参照リンクと詳細解説ページ（[35_v216Tangent.md](../01_features/35_v216Tangent.md)）を追加。前回 2026-07-25: v2.14.2・v2.14.1・v2.14.0・v2.13.1対応追加、v2.13.0・v2.12.3・v2.12.2・v2.12.1 をバージョン履歴へ移動、冒頭注記を v2.14.2 に更新、v2.12.3・v2.6.0 に v2.14.1 の方針転換（セッション限定化）の相互参照追加、v2.13.0 に v2.13.1 の相互参照追加）
