...
Spec agent は次の 3 フェーズで進み、各フェーズで `.kiro/specs/<name>/` 配下にファイルを生成します。フェーズの区切りでユーザーが内容を編集できます。

| フェーズ | 内容 | 生成ファイル |
|---------|------|-------------|
| **Requirements**（要件） | 受入基準を含む要件を定義 | `.kiro/specs/<name>/requirements.md` |
| **Design**（設計） | アーキテクチャ・設計を記述 | `.kiro/specs/<name>/design.md` |
| **Tasks**（タスク） | 依存関係を追跡した順序付き実装計画 | `.kiro/specs/<name>/tasks.md` |
| **Execution**（実行） | タスクを**逐次**実行し、各ステップ間で検証 | （上記ファイルを更新しながら進行） |
