...
> **V2 TUI で追加**: このコマンドは V2 TUI モード（v2.0.0+ デフォルト）で追加されました。macOS・Windows（PowerShell）・Linux で動作します。Legacy UI（`--classic`）では利用できません。
...
> **v2.11.0+**: プリペイドの「Additional credits」パックを表示（旧 post-paid 超過分に代替）。使用上限（cap）が無いユーザーではプログレスバーを非表示。
...
> /mcp auth                           # OAuth 再認証を強制（v2.11.0+）
> /mcp cancel-auth                    # 保留中の認証フローを中止（v2.11.0+）
> /mcp logout                         # 保存済み OAuth 資格情報を削除（v2.11.0+）
```
...
- `auth` — リモート MCP サーバーの OAuth 再認証を強制（トークン失効・無効時）（**v2.11.0+**）
- `cancel-auth` — ブラウザ確認待ちで停止した認証フローを中止（**v2.11.0+**）
- `logout` — 保存済み OAuth 資格情報を削除（**v2.11.0+**）
...
- **有効化**: `/settings display` → Terminal title、または `kiro-cli settings chat.terminalTitle true`（v2.7.0+。無効時は `/title` が有効化方法を案内）

> **注**: v2.6.0 時点ではターミナルタイトルは `/settings display` でトグルする機能のみで CLI 設定としては提供されていませんでしたが、**v2.7.0 で `chat.terminalTitle` 設定が追加**されました（boolean、既定 `false`。実機 kiro-cli 2.10.0 の `kiro-cli settings list --all` で確認）。→ [01_settings.md](01_settings.md)、[29. v2.7 新コマンド](../01_features/29_v27NewCommands.md)
...
> **V2 TUI で追加**: このコマンドは v2.4.0 で追加されました。TUI モード（デフォルト）で利用可能です。Legacy UI（`--classic`）では利用できません。
