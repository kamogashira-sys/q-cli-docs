...
**Agent Toolkit for AWS** は、AI コーディングエージェントが AWS サービス上で **効果的かつ安全に** 動作するための公式 AWS 製品です。AWS の **公式 MCP Server**、**Skills**（タスク別ガイド）、**Plugins**、**Rules files** を統合的に提供します。

**2026 年 5 月 6 日に AWS MCP Server が GA リリース** されました。

#### 公式が対応として明示しているエージェント

[User Guide §What can I do with the Agent Toolkit for AWS?](https://docs.aws.amazon.com/agent-toolkit/latest/userguide/what-is-agent-toolkit.html#what-can-i-do-agent-toolkit) より：

> The Agent Toolkit for AWS works with **Claude Code, Cursor, Codex, Kiro, Windsurf, Cline**, and any other agent that supports the Model Context Protocol.

加えて [AWS News Blog（2026-05-06）](https://aws.amazon.com/blogs/aws/the-aws-mcp-server-is-now-generally-available) では **「Kiro CLI、Kiro」** と両方が公式対応として明記されています。
...
- **GA 時点の規模**（[What's New: Agent Toolkit](https://aws.amazon.com/about-aws/whats-new/2026/05/agent-toolkit/) より）:
  - **40+ Skills 公開中**（Infrastructure-as-Code、ストレージ、分析、サーバーレス、コンテナ、AI サービス）
  - 今後 **データベース、ネットワーキング、IAM** 用も順次追加予定
- **特徴**:
...
- **対象（出典別）**:
  - [User Guide §Components](https://docs.aws.amazon.com/agent-toolkit/latest/userguide/what-is-agent-toolkit.html#agent-toolkit-components) および [GitHub README](https://github.com/aws/agent-toolkit-for-aws#plugins): **Claude Code、Codex のみ**
  - [製品ページ Features](https://aws.amazon.com/jp/products/developer-tools/agent-toolkit-for-aws/): Claude Code、Codex、**Cursor も含めて記載**（先行アナウンスの可能性）
- **内容**: AWS MCP Server 設定 + 厳選 Skills セット
- **特徴**:
  - 単一インストールで MCP Server + Skills が有効化
  - 自動更新（Skills と MCP Server 設定が新機能追加時に更新）

#### 公式 3 Plugin（2026-05 GA 時点）

[GitHub: aws/agent-toolkit-for-aws/plugins](https://github.com/aws/agent-toolkit-for-aws/blob/main/plugins) に公開：

| Plugin 名 | 内容 |
|---------|------|
| [`aws-core`](https://github.com/aws/agent-toolkit-for-aws/blob/main/plugins/aws-core) | サービス選定、CDK/CloudFormation、サーバーレス、コンテナ、ストレージ、可観測性、課金、SDK、デプロイ — **まずはここから** |
| [`aws-agents`](https://github.com/aws/agent-toolkit-for-aws/blob/main/plugins/aws-agents) | Bedrock + AgentCore で AI エージェントを構築する skills |
| [`aws-data-analytics`](https://github.com/aws/agent-toolkit-for-aws/blob/main/plugins/aws-data-analytics) | S3 Tables、AWS Glue、Athena による分析・ETL |
...
> **Kiro / Kiro CLI**: 公式 Plugin は未対応（2026-05 時点、GitHub README 明記）。代わりに **`mcp.json` 直接設定 + `npx skills add`** で利用します（後述「[Kiro CLI 統合手順](#kiro-cli-統合手順)」参照）。
...
> **公式ドキュメントの見出しは「Kiro」**（無印）です。Kiro（IDE）と Kiro CLI は **同じ設定ディレクトリ `~/.kiro/`** を共有するため、`mcp.json` の置き場所と書き方は共通です。本節では本サイトの目的に合わせて Kiro CLI からの利用手順を記述します。
...
> **`mcpServers` キー名について**: 上記の例では `"aws-mcp"` を使用しています（[製品ページ Kiro セクション](https://aws.amazon.com/jp/products/developer-tools/agent-toolkit-for-aws/) 準拠）。一方 [GitHub: aws/agent-toolkit-for-aws の README](https://github.com/aws/agent-toolkit-for-aws) では `"aws"` が使用されています。これはエージェントが MCP サーバーを識別するためのローカル名で、**任意の名前**を指定できます（既存の MCP サーバー名と衝突しなければ何でも可）。本サイトでは製品ページに合わせて `"aws-mcp"` を採用します。
...
Strands Agents、LangChain、LlamaIndex、Microsoft Agent Framework など自社エージェントに AWS 操作能力を付与できます。

[`aws/mcp-proxy-for-aws`](https://github.com/aws/mcp-proxy-for-aws) は **Programmatic Access** ライブラリも提供しており、`aws_iam_streamablehttp_client` を使うことで MCP Proxy をプログラム内に組み込めます。

#### Strands Agents での実装例（公式パターン）
...
| フレームワーク | 公式サンプル |
|--------|--------|
| Strands Agents | [examples/mcp-client/strands](https://github.com/aws/mcp-proxy-for-aws/tree/main/examples/mcp-client/strands) |
| LangChain | [examples/mcp-client/langchain](https://github.com/aws/mcp-proxy-for-aws/tree/main/examples/mcp-client/langchain) |
| LlamaIndex | [examples/mcp-client/llamaindex](https://github.com/aws/mcp-proxy-for-aws/tree/main/examples/mcp-client/llamaindex) |
| Microsoft Agent Framework | [examples/mcp-client/agent-framework](https://github.com/aws/mcp-proxy-for-aws/tree/main/examples/mcp-client/agent-framework) |

→ サンドボックス Python 実行（`run_script`）と組み合わせれば、ファイルシステム・ネットワークなしで複雑な分析も安全に実行できます。
...
3. Kiro CLI で `/mcp` スラッシュコマンドで状態確認 → エラーログ確認（→ [04_reference/02_slash-commands.md](../04_reference/02_slash-commands.md)）
4. `mcp-proxy-for-aws` 単体起動で疎通確認:
   ```bash
   uvx mcp-proxy-for-aws@latest https://aws-mcp.us-east-1.api.aws/mcp --log-level DEBUG
   ```
5. Kiro CLI 自体のログ・設定の保存先は [04_reference/01_settings.md](../04_reference/01_settings.md) を参照
...
**Page updated**: 2026-05-24（本サイト初版 + 一次情報レビュー反映）  
**公式 GA**: 2026-05-06
