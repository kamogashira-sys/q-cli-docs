[ホーム](../README.md) > [機能詳細ガイド](README.md) > Granular URL Permissions
...
### ユースケース2〜4: パターンの差し替えだけで応用可能

JSON の構造はユースケース1と同一で、`trusted` / `blocked` のパターンだけが変わります:

| ユースケース | trusted（自動許可） | blocked（自動拒否） | ねらい |
|------------|-------------------|-------------------|--------|
| **2. エンタープライズ環境** | `^https://docs\.company\.com/.*`<br/>`^https://wiki\.company\.com/.*`<br/>`^https://internal\.company\.com/.*` | `^https://(?!.*\.company\.com).*`<br/>`^http://.*` | 社内ドメインのみ許可、外部は完全ブロック |
| **3. オープンソースプロジェクト** | `^https://github\.com/.*`<br/>`^https://raw\.githubusercontent\.com/.*`<br/>`^https://docs\..*`<br/>`^https://registry\.npmjs\.org/.*`<br/>`^https://pypi\.org/.*`<br/>`^https://crates\.io/.*` | （なし） | GitHub・公式ドキュメント・パッケージレジストリへの自動アクセス |
| **4. セキュリティ重視** | `^https://docs\.aws\.amazon\.com/.*`<br/>`^https://github\.com/.*` | `^https://malicious-site\.com/.*`<br/>`^https://.*\.suspicious\.com/.*`<br/>`^http://.*` | 最小限の許可＋既知の不正サイトと HTTP を自動ブロック（HTTPS 強制） |

> **ポイント**: `blocked` は `trusted` より優先されます。HTTP を全面ブロックする `^http://.*` は多くの環境で有効な defensive パターンです。
...
**出典**: [Enterprise Governance - Web tools](https://kiro.dev/docs/enterprise/governance/web-tools/)（公式サイトはEnterprise Settingsページの表内項目からGovernance配下の専用ページへ分離。`enterprise/settings/`にも同項目の要約が残るが見出しアンカーはなし）
...
### リファレンス（辞書）
- [04_reference/01_settings.md](../04_reference/01_settings.md) — Web tools 関連設定（`web.*`、Enterprise governance）
- [04_reference/04_built-in-tools.md](../04_reference/04_built-in-tools.md) — `web_fetch` `web_search` ツールの正規仕様（`trusted` `blocked` パターン、Fetch モード）

### 関連機能
- [17. Granular Tool Trust](17_GranularToolTrust.md) — `web_*` ツールへの段階的信頼制御

---

**最終更新**: 2026-07-04
**対象バージョン**: Kiro CLI v1.24.0+（v1.25.0 Enterprise Web Tools Governance を含む）
