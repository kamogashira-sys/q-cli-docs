[ホーム](../README.md) > [機能詳細ガイド](README.md) > Grep/Glob ツール
...
### サブエージェントでの利用（v2.x で解消済み）

> **旧制約（v1.23 時点）**: 導入当初、Grep・Glob ツールはサブエージェントでは利用できず、ファイル検索には `shell` ツール（bash コマンド）が使われていました。

現在は、公式 [Subagents - Tool availability](https://kiro.dev/docs/custom-agents/subagents/) が「**デフォルトサブエージェントはメインエージェントと同じ組み込みツールを持つ**」と明記しており、`grep`・`glob` もサブエージェント内で利用できます（2026-07-04 確認）。

**注意点**:
- **カスタムエージェント**をサブエージェントとして指定した場合は、そのエージェント設定の `tools` に列挙されたツールだけが使えます。`grep`/`glob` を設定に含めていないカスタムエージェントでは利用できません。
...
### 関連機能
- [24. @file references](24_FileReferences.md) 🆕 — `@directory` でのディレクトリ展開と `glob`/`grep` の使い分け
- [09. AST Pattern Tools](09_ASTPatternTools.md) — 構文木ベースの精密検索
- [22. Smart Hooks](22_Hooks.md) 🆕 — `glob`/`grep` の matcher 指定例

### リファレンス（辞書）
- [04_reference/04_built-in-tools.md](../04_reference/04_built-in-tools.md) — `glob`、`grep` ツールの正規仕様（`allowedPaths`/`deniedPaths`/`allowReadOnly`）

---

**最終更新**: 2026-07-04
**対象バージョン**: Kiro CLI v1.23.0+
