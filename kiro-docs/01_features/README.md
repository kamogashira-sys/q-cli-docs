...
**バージョンごとの新機能・変更点・バグ修正の時系列一覧は [02_update/01_changelog.md](../02_update/01_changelog.md) に集約しています**（各エントリから該当の機能詳細ページへ「📖 詳細解説」リンクあり）。本ページの[機能一覧表](#-機能一覧リリース情報付き)の「リリース」列でも、各機能の導入・更新バージョンを確認できます。
