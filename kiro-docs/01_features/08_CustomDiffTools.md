[ホーム](../README.md) > [機能詳細ガイド](README.md) > Custom Diff Tools
...
どのツールも、インストール済みであれば `kiro-cli settings chat.diffTool <設定値>` を 1 回実行するだけで切り替えられます。
各ツールのインストール手順は OS・パッケージマネージャごとに変わるため、公式ページを参照してください（常に最新の手順が掲載されています）。

#### ターミナルツールの設定コマンドと入手先

| ツール | 設定コマンド | 入手先（公式） |
|--------|------------|--------------|
| delta | `kiro-cli settings chat.diffTool delta` | [dandavison/delta](https://github.com/dandavison/delta) |
| difftastic | `kiro-cli settings chat.diffTool difft` | [Wilfred/difftastic](https://github.com/Wilfred/difftastic) |
| icdiff | `kiro-cli settings chat.diffTool icdiff` | [jeffkaufman/icdiff](https://github.com/jeffkaufman/icdiff) |
| diff-so-fancy | `kiro-cli settings chat.diffTool diff-so-fancy` | [so-fancy/diff-so-fancy](https://github.com/so-fancy/diff-so-fancy) |
| colordiff | `kiro-cli settings chat.diffTool colordiff` | [colordiff.org](https://www.colordiff.org/) |
| diff-highlight | `kiro-cli settings chat.diffTool diff-highlight` | Git 同梱（追加インストール不要） |
| ydiff | `kiro-cli settings chat.diffTool ydiff` | [ymattw/ydiff](https://github.com/ymattw/ydiff) |
| bat | `kiro-cli settings chat.diffTool bat` | [sharkdp/bat](https://github.com/sharkdp/bat) |

各ツールの特徴（構文ハイライト・サイドバイサイド表示等）は前掲の「サポートされるツール」の表を参照してください。
...
| ツール | 設定コマンド | 入手先（公式） |
|--------|------------|--------------|
| VS Code | `kiro-cli settings chat.diffTool code` | [code.visualstudio.com](https://code.visualstudio.com/) |
| VSCodium | `kiro-cli settings chat.diffTool codium` | [vscodium.com](https://vscodium.com/) |
| Meld | `kiro-cli settings chat.diffTool meld` | [meldmerge.org](https://meldmerge.org/) |
| KDiff3 | `kiro-cli settings chat.diffTool kdiff3` | [kdiff3.sourceforge.net](https://kdiff3.sourceforge.net/) |
| FileMerge (macOS) | `kiro-cli settings chat.diffTool opendiff` | Xcode Command Line Tools（`xcode-select --install`） |
| Vim | `kiro-cli settings chat.diffTool vimdiff`（または `vim`） | 多くのシステムにプリインストール済み |
| Neovim | `kiro-cli settings chat.diffTool nvim` | [neovim.io](https://neovim.io/) |
...
**例: deltaのインストール**（他ツールは「各ツールの設定方法」の表にある公式ページを参照）
...
**delta の表示イメージ**（例示。実際の描画は delta のバージョン・テーマ設定により異なります）:
...
**delta の表示イメージ**（例示。実際は構文ハイライト・行番号付きで描画されます）:
...
**difftastic の表示イメージ**（例示。実際のメッセージ文言はバージョンにより異なります）:
...
**icdiff の表示イメージ**（例示。実際はカラー付きの左右2カラムで描画されます）:
...
**最終更新**: 2026-07-04
**対象バージョン**: Kiro CLI v1.24.0+（Classic モード。Terminal UI では外部 Diff ツール未対応）
