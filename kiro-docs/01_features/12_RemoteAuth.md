[ホーム](../README.md) > [機能詳細ガイド](README.md) > Remote Authentication
...
Kiro CLI v1.24.0（2026年1月16日リリース）で追加されたRemote Authentication機能について詳細に解説します。v1.25.1（2026年2月12日リリース）では、**External Identity Provider対応**（Okta/Microsoft Entra ID）が追加され、Enterprise SSOが利用可能になりました。
...
- **External Identity Provider対応（v1.25.1）**: Okta/Microsoft Entra IDによるEnterprise SSO
...
#### External Identity Provider（v1.25.1で追加）

| 認証方法 | 環境 | 対応 |
|---------|------|------|
| Okta SSO | すべて | ✅ v1.25.1 |
| Microsoft Entra ID SSO | すべて | ✅ v1.25.1 |
...
graph TD
    ...
    C -->|Okta/Entra ID| F2[ブラウザベースOAuth]
    D --> F[ローカルブラウザで認証]
    E --> G[デバイスコードを入力]
    F2 --> F
    F --> H[認証完了]
  ...
  build:
    ...
    steps:
      ...
      - name: Authenticate with Builder ID
        run: |
          # デバイスコード認証（free = Builder ID / Social login）
...
| Okta SSO | すべて | ブラウザベースOAuth（v1.25.1） |
| Microsoft Entra ID SSO | すべて | ブラウザベースOAuth（v1.25.1） |
...
### リファレンス（辞書）
- [04_reference/03_cli-commands.md](../04_reference/03_cli-commands.md) — `kiro-cli login`（`--use-device-flow` フラグ等）、`kiro-cli logout`、`kiro-cli whoami` の正規仕様

### 関連機能
- [03_deployment/03_official-installation.md](../03_deployment/03_official-installation.md) 🆕 — Linux/macOS/Windows のインストール手順とリモート認証

---

**最終更新**: 2026-07-04
**対象バージョン**: Kiro CLI v1.24.0+（v1.25.1 External IdP・v2.1.0 Device Flow を含む）
