[ホーム](../README.md) > [機能詳細ガイド](README.md) > Tool Search
...
2. **BM25キーワードマッチング** — ツール名・説明からの関連性スコアリング（BM25 は検索エンジンで広く使われるキーワード関連度の計算方式）
...
### 本サイトの関連文書
- [22. Smart Hooks](22_Hooks.md) 🆕 — Hook の matcher で `tool_search` を対象指定可能
- [26. Agent Toolkit for AWS](26_AgentToolkitForAWS.md) 🌟 — AWS 公式 MCP Server（Tool Search 対象として利用可能）
- [04_reference/01_settings.md](../04_reference/01_settings.md) — `toolSearch.*` 設定（`enabled`/`maxToolDescriptions`/`KIRO_CLI_TOOL_SEARCH_MATCHING_THRESHOLD` 環境変数）
- [04_reference/04_built-in-tools.md](../04_reference/04_built-in-tools.md) — `tool_search` ツールの正規仕様（`tool_id`/`query`/`max_results` パラメータ）

---

**最終更新**: 2026-07-04
**対象バージョン**: Kiro CLI v2.1.0+
